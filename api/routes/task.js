...
router.get('/:userId', checkAuth, function (req, res, next) {
    ...
    connection
        ...
        INNER JOIN
            project on task.projectId = project.id
        INNER JOIN
            kpi on task.workType = kpi.id
        WHERE task.assignTo = "${userId}"`,
            (err, result) => {
                if (err) res.status(500).json({ message: err });
                res.status(200).json(result);
            });
...
router.get('/assignedTask/:userId', checkAuth, function (req, res, next) {
    const userId = req.params.userId;
    connection
        .query(`
        select 
...
router.post('/searchTaskByDate/:id', function (req, res) {
    ...
    connection.query(
        ...
        INNER JOIN
            kpi on task.workType = kpi.id
        WHERE task.assignDate >= "${req.body.startDate}" AND task.assignDate <= "${req.body.endDate}" AND task.assignTo="${userId}"`,
        (err, result) => {
            if (err) {
                res.status(404).json({
                    message: "Error in Task Date Range search",
                    success: false,
                    error: err
                })
            } else {
                res.status(200).json({
                    message: "Task Date Range search Success",
                    count: result.length,
                    success: true,
                    result: result
                })
